...
import { BookingService, HandyService, LocalStorageService, LoaderService } from '../../../providers';
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      ...
      if (serviceList) {
        ...
        const services = serviceList.filter(item => item.selected === true)
          .reduce(function (a, b) {
            return a + b.name + ', ';
          }, '');
    ...
    this.userName = this.localStorageService.getItem(GLOBAL_CONSTANTS.LS_LOGIN_USER_DATA, 'name');
    this.formData.location = this.localStorageService.getItem(GLOBAL_CONSTANTS.LS_CITY);
  }

  retrieveServiceName(service_id) {
    this.handyService.getServiceName(service_id)
    ...
    this.modalService.show(modalContent)
      .then(result => {
        if (result === true) {
          ...
          this.bookingService.bookServices(req)
            .subscribe(res => {
              if (res) {
