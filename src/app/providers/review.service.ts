...
export class ReviewService extends BaseService {
  ...
  getReviews(): Observable<Review[]> {
    ...
    return this.http
      .get(`${GLOBAL_CONSTANTS.BASE_API_URL}/reviews`)
      .map(res => {
        const result = res.json();
        return this.buildReviewData(result.slice(0, this.MAX_REVIEW));
      })
      .catch((error: any) =>
  ...
  getReviewsByService(service_id: string): Observable<Review[]> {
    ...
    return this.http
      .get(
        `${GLOBAL_CONSTANTS.BASE_API_URL}/service/reviews?service_id=${service_id}`
      )
      .map(res => {
        const result = res.json();
        return this.buildReviewData(result.slice(0, this.MAX_REVIEW));
  ...
  addReview(req: ReviewReq): Observable<string> {
    ...
    return this.http
      .post(`${GLOBAL_CONSTANTS.BASE_API_URL}/review`, bodyString, options)
  ...
  buildReviewData(result: any): Review[] {
    ...
    const chunks = [];
    for (let i = 0; i < reviewList.length; ) {
      chunks.push(reviewList.slice(i, (i += 4)));
    }
    return chunks;
  }
}
