...
export class AuthService extends BaseService {
  ...
  public registerUser(user: RegisterUserRequest): Observable<RegisterUserResponse> {
    ...
    return this.http.post(`${GLOBAL_CONSTANTS.BASE_API_URL}/user/register`, bodyString, options)
      .map(res => {
  ...
  public loginUser(user: LoginUserRequest): Observable<LoginUserResponse> {
    ...
    const headers = new Headers({ 'Content-Type': 'application/json' });
    const options = new RequestOptions({ headers: headers });
    return this.http.post(`${GLOBAL_CONSTANTS.BASE_API_URL}/user/login`, bodyString, options)
      .map(res => {
        const response = res.json();
  ...
  public resetPassword(email: string): Observable<string> {
    ...
    return this.http.post(`${GLOBAL_CONSTANTS.BASE_API_URL}/user/forgot-password`, bodyString, options)
      .map(res => {
  ...
  public authGoogle(token: string): Observable<any> {
    ...
    const token_request = `code=${token}&client_id=${this.authConfig.google.clientId}&client_secret=${this.authConfig.google.client_secret}&redirect_uri=${this.authConfig.google.redirectURI}&grant_type=authorization_code`;
    const request_length = token_request.length;
    const bodyString = token_request;
    const headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
    const options = new RequestOptions({ headers: headers });

    return this.http.post(accessTokenUrl, bodyString, options)
      .map(res => {
  ...
  private loginUserWithGoogle(id: string, email: string, name: string): Observable<any> {
    ...
    return this.http.post(`${GLOBAL_CONSTANTS.BASE_API_URL}/user/login/google?id=${id}`, bodyString, options)
      .map(res => res.json())
      .catch(err => super.handleError(err));
  }
  ...
  private loginUserWithFacebook(id: string, email: string, name: string): Observable<any> {
    ...
    const bodyString = JSON.stringify(req);
    return this.http.post(`${GLOBAL_CONSTANTS.BASE_API_URL}/user/login/fb?id=${id}`, bodyString, options)
      .map(res => {
  ...
  private getGoogleProfile(accessToken: string): Observable<any> {
    ...
    return this.http.get(peopleApiUrl, options)
      .map(res => res.json())
      .catch((error: any) => Observable.throw(error.json().error || 'Server Error'));
  }
  ...
  public authFacebook(token: string): Observable<any> {
    ...
    const token_request = `code=${token}&client_id=${this.authConfig.facebook.clientId}&client_secret=${this.authConfig.facebook.client_secret}&redirect_uri=${this.authConfig.facebook.redirectURI}`;
    // const bodyString = JSON.stringify(params);
    this.logger.log('authFacebook-req', token_request);
    const headers = new Headers({ 'Content-Type': 'application/x-www-form-urlencoded' });
    const options = new RequestOptions({ headers: headers });
    return this.http.post(accessTokenUrl, token_request, options)
      .map(res => {
  ...
  public updateUser(user_id: string, mobile: string): Promise<string> {
    const req = {
      number: mobile,
      verified: true
    };
    ...
    const options = new RequestOptions({ headers: headers });
    this.logger.log(bodyString);
    return this.http.post(`${GLOBAL_CONSTANTS.BASE_API_URL}/user/update?id=${user_id}`, bodyString, options)
      .toPromise()
  ...
  public sendOtp(mobile: string): Observable<any> {
    ...
    return this.http.post(`${GLOBAL_CONSTANTS.BASE_API_URL}/send/otp`, bodyString, options)
      .map(res => res.json())
      .catch(err => super.handleError(err));
  }
  ...
  public verifyOtp(otp: string, otpId: string): Observable<any> {
    ...
    return this.http.post(`${GLOBAL_CONSTANTS.BASE_API_URL}/verify/otp`, bodyString, options)
      .map(res => res.json())
      .catch(err => super.handleError(err));
    /*response.message
      'not matched'
